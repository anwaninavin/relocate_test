...
export function CategoryManagerDialog({
  ...
  const [moveTargetFor, setMoveTargetFor] = useState<{ id: string; itemCount: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!open) return;
    setIsLoading(true);
    listCategoriesAction().then((result) => {
      setIsLoading(false);
      if (result.success) {
  ...
  async function handleDelete(id: string, moveItemsTo?: string) {
    setIsSubmitting(true);
    const result = await deleteCategoryAction({ id, moveItemsTo });
    setIsSubmitting(false);

    if (!result.success) {
      if (result.moveRequired) {
        setMoveTargetFor({ id, itemCount: result.itemCount ?? 0 });
        return;
      }
      toast.error(result.error);
      return;
