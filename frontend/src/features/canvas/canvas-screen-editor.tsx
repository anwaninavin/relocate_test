...
export interface CanvasScreenEditorProps {
  /** Card heading shown above the canvas, e.g. "Home screen" or "Guide screen". */
  title: string;
  /** Which landing page this editor saves to (`page` field on the backend design doc). */
  page: string;
  sections: HomeSectionDef[];
  defaultElements: CanvasElement[];
  mergeElements: (overrides: ElementOverride[] | null | undefined) => CanvasElement[];
  diffElements: (edited: CanvasElement[]) => ElementOverride[];
  mergeBackgrounds: (overrides: SectionBackgroundOverride[] | null | undefined) => Record<string, string>;
  diffBackgrounds: (edited: Record<string, string>) => SectionBackgroundOverride[];
}

export function CanvasScreenEditor({
  title,
  page,
  sections,
  defaultElements,
  ...
  mergeBackgrounds,
  diffBackgrounds,
}: CanvasScreenEditorProps) {
  const [elements, setElements] = useState<CanvasElement[]>(defaultElements);
  const [sectionBackgrounds, setSectionBackgrounds] = useState<Record<string, string>>(() => mergeBackgrounds(null));
  ...
  useEffect(() => {
    api
      .get<{ elements: ElementOverride[] | null; sectionBackgrounds: SectionBackgroundOverride[] | null }>(
        `/api/landing/design?page=${encodeURIComponent(page)}`,
      )
      .then((res) => {
  ...
  function resetElement(id: string) {
    const base = defaultElements.find((e) => e.id === id);
    if (!base) return;
  ...
  async function handleSave() {
    setIsSaving(true);
    try {
      const overrides = diffElements(elements);
      const backgroundOverrides = diffBackgrounds(sectionBackgrounds);
      await api.put("/api/admin/landing-design", { page, elements: overrides, sectionBackgrounds: backgroundOverrides });
                ...
                <div className="flex flex-wrap gap-2">
                  {hiddenElements.map((e) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        const elementSection = sections[e.section];
                    ...
                    >
                      ...
                      {elementLabel(e)}
                      <span className="text-muted-foreground">· {sections[e.section]?.label ?? "?"}</span>
                    </button>
                  ))}
                </div>
              </div>
